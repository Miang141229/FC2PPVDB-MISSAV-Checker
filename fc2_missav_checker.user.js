...
(function () {
    ...
    if (window.location.href.startsWith('https://fc2ppvdb.com/articles/')) {
        ...
        if (match && match[1]) {
            ...
            const missavSearchUrl = `https://missav.ai/search/${articleId}`;
            ...
            GM_xmlhttpRequest({
                ...
                headers: {
                    ...
                    "Referer": "https://missav.ai/" // 模擬從 missav123.com 跳轉過去
                },
            ...
            function updateResultDisplay(message) {
                ...
                if (message.includes('沒有找到')) {
                    ...
                    const link = message.match(/(https:\/\/missav\.ai\/search\/\d+)/);
                    if (link && link[1]) {
                        const url = link[1];
